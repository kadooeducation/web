...
const _initialPendingMentors: PendingMentor[] = [
  {
    id: 101,
    name: 'Ana Silva',
    area: 'Marketing',
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-600 mt-2">
        Total de mentores: <span className="font-medium">{total}</span>
      </p>
      <div className="mt-2 flex flex-wrap gap-2">
        {mentorAreas.map((m) => (
          <span
            key={m.area}
            className="inline-flex items-center gap-2 text-xs bg-gray-50 rounded-full px-2.5 py-1"
          >
            <span
