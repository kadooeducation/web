...
import { ActivityResponse, ActivityResponseGateway } from "./activity-response-gateway";
...
export class ActivityResponseGatewayHttp implements ActivityResponseGateway {
  ...
  async userAlreadyResponseByStepId(stepId: number): Promise<boolean> {
    const result = await this.client.get<boolean>(`/activity-response/${stepId}`)
...
export const activityResponseGatewayHttp = new ActivityResponseGatewayHttp(client)